...
class CryptoApi {
    ...
    async getPriceData(coinOption, currOption) {
        try {
            ...
            //formatter currency
            const formatter = new Intl.NumberFormat('en-US', { style: "currency", currency: currOption });

            let res = await axios.get(`${this.baseUrl}?key=${this.apiKey}&ids=${coinOption}&convert=${currOption}`)
            let output = ``;
